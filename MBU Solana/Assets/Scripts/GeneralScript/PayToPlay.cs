    ...
    public class PayToPlay : MonoBehaviour
    {
        ...
        public async void TryPayToPlay(ulong requiredAmount, Action onSuccess, Action<string> onFailure, string actionType = "")
        {
            ...
            */
            ...
            if (res.Result == null)
            {
                Debug.Log("The res result is null");
                return;
            }
            else if (res.Result.Value == null)
            {
                Debug.Log("The res result Value is null");
                return;
            }
            else
            {
                Debug.Log("The UI Amount");
                Debug.Log(res.Result.Value.UiAmountString);
            }
            ulong userBonkAmount = res.Result.Value.AmountUlong;//bonkTokenAccount.Account.Data.Parsed.Info.TokenAmount.AmountUlong;
