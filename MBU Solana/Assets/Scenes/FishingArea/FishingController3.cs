...
using UnityEngine;
public class FishingController3 : MonoBehaviour
{
    // Fishing rods
    ...
    // Fishing rod data
    [Header("Fishing Rod Chances")]
    public float[] rodChances = { 0.1f, 0.2f, 0.3f, 0.4f, 0.5f };

    // Bait data
    [Header("Bait Luck Values")]
    public float[] baitLuckValues = { 0.1f, 0.2f, 0.3f, 0.69f, 0.5f };
    ...
    {
        return rodChances[(int)rodType];
    }

    private float GetBaitLuck(BaitType baitType)
    {
        return baitLuckValues[(int)baitType];
    }
}
